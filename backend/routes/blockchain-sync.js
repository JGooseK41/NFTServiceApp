        ...
        try {
            ...
            await client.query(`
                INSERT INTO served_notices (
                    ...
                    notice_type, issuing_agency, alert_id, document_id, has_document, created_at
                ) VALUES (
                    'notice_123456_alert', '123456', $1,
                    'TFfagVe1aZpSfYaruY6xJfVPYZBuMj57FH',
                    'Legal Notice', 'Court Agency', '1', '2', true,
                    NOW() - INTERVAL '2 days'
            ...
            await client.query(`
                INSERT INTO served_notices (
                    ...
                    'notice_34987654_alert', '34-987654', $1,
                    'TD1F37V4cAFH1YQCYVLtcFyFXkZUs7mBDE',
                    'Notice of Seizure', 'The Block Audit', '3', '4', true,
                    NOW() - INTERVAL '1 day'
                ) ON CONFLICT (notice_id) DO UPDATE
                SET server_address = EXCLUDED.server_address,
                    case_number = EXCLUDED.case_number,
            ...
            // Get the synced cases to return (can use pool for read-only)
            const result = await pool.query(`
                SELECT case_number, server_address, recipient_address,
                    alert_id, document_id, accepted, created_at
                FROM served_notices
