            ...
            for (const caseData of cases) {
                ...
                if (recipients.some(r => r.toLowerCase() === walletAddress.toLowerCase())) {
                    myNotices.push({
                        ...
                        issuing_agency: caseData.agency || caseData.metadata?.agency || caseData.metadata?.issuingAgency || 'via Blockserved.com',
    ...
    for (const caseData of localCases) {
        ...
        if (recipients.some(r => r.toLowerCase() === walletAddress.toLowerCase())) {
            myLocalNotices.push({
                ...
                notice_type: caseData.noticeType || 'Legal Notice',
                issuing_agency: caseData.agency || caseData.issuingAgency || 'via Blockserved.com',
