...
contract LegalNoticeNFT {
    ...
    // Temporary structs to avoid stack depth
    ...
    // Temporary storage to avoid stack issues
    CreateNoticeParams private _tempCreateParams;
    ServeNoticeParams private _tempServeParams;
    ...
    // Split createLegalNotice into two parts to avoid stack depth
    function createLegalNotice(
        ...
        // Store params in temporary storage
        _tempCreateParams.recipient = recipient;
        _tempCreateParams.ipfsHash = ipfsHash;
        _tempCreateParams.previewImage = previewImage;
        _tempCreateParams.contentHash = contentHash;
        _tempCreateParams.caseNumber = caseNumber;
        _tempCreateParams.jurisdictionIndex = jurisdictionIndex;
        _tempCreateParams.documentType = documentType;
        
        // Call internal function with no parameters
        return _createLegalNoticeInternal();
    }
    
    function _createLegalNoticeInternal() private returns (uint256 noticeId, uint256 alertId) {
        require(msg.value >= serviceFee + 2000000, "Insufficient fee");
        require(_tempCreateParams.recipient != address(0), "Invalid recipient");
        
        // Create document
        noticeId = ++_documentCounter;
        documents[noticeId].server = msg.sender;
        documents[noticeId].recipient = _tempCreateParams.recipient;
        documents[noticeId].ipfsHash = _tempCreateParams.ipfsHash;
        documents[noticeId].caseInfo = string(abi.encodePacked("Legal Authority - Legal Notice - Case ", _tempCreateParams.caseNumber));
        documents[noticeId].timestamp = block.timestamp;
        documents[noticeId].decryptionKey = "";
        recipientDocuments[_tempCreateParams.recipient].push(noticeId);
        
        // Create alert  
        alertId = ++_alertCounter;
        alerts[alertId].documentId = noticeId;
        alerts[alertId].recipient = _tempCreateParams.recipient;
        alerts[alertId].issuingAgency = "Legal Authority";
        alerts[alertId].noticeType = "Legal Notice";
        alerts[alertId].caseNumber = _tempCreateParams.caseNumber;
        alerts[alertId].caseDetails = "Document attached";
        alerts[alertId].legalRights = "You have legal rights regarding this notice";
        alerts[alertId].feesSponsored = true;
        alerts[alertId].timestamp = block.timestamp;
        recipientAlerts[_tempCreateParams.recipient].push(alertId);
        _tokenOwners[alertId] = _tempCreateParams.recipient;
        
        // Create notice
        notices[noticeId].recipient = _tempCreateParams.recipient;
        notices[noticeId].server = msg.sender;
        notices[noticeId].ipfsHash = _tempCreateParams.ipfsHash;
        notices[noticeId].contentHash = _tempCreateParams.contentHash;
        notices[noticeId].timestamp = uint128(block.timestamp);
        notices[noticeId].caseNumberHash = uint64(uint256(keccak256(abi.encodePacked(_tempCreateParams.caseNumber))) >> 192);
        notices[noticeId].alertTokenId = uint32(alertId);
        notices[noticeId].jurisdictionIndex = _tempCreateParams.jurisdictionIndex;
        notices[noticeId].documentType = _tempCreateParams.documentType;
        notices[noticeId].status = 0;
        
        emit Transfer(address(0), _tempCreateParams.recipient, alertId);
        emit AlertCreated(alertId, _tempCreateParams.recipient, noticeId);
    }
    ...
    // Split serveNotice to avoid stack depth issues
    function serveNotice(
        ...
        // Now call internal function to populate data
        _populateServeNoticeData(
            documentId,
            alertId,
            recipient,
            encryptedIPFS,
        ...
        // Complete in second function
        _completeServeNotice(
            documentId,
            alertId,
            recipient,
            caseDetails,
    ...
    function _populateServeNoticeData(
        uint256 documentId,
        uint256 alertId,
        address recipient,
        string calldata encryptedIPFS,
        ...
        documents[documentId].ipfsHash = encryptedIPFS;
        documents[documentId].caseInfo = string(abi.encodePacked(issuingAgency, " - ", noticeType, " - Case ", caseNumber));
        ...
        notices[documentId].ipfsHash = encryptedIPFS;
        notices[documentId].contentHash = keccak256(abi.encodePacked(encryptedIPFS));
