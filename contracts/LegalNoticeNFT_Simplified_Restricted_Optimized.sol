...
contract LegalNoticeNFT_Simplified is ERC721, ERC721URIStorage, AccessControl, ReentrancyGuard {
    ...
    function createDocumentNotice(
        ...
        _setTokenURI(noticeId, generateTokenURI(noticeId));
    ...
    // Register/update process server
    function registerProcessServer(string memory name, string memory agency) external {
        ...
        require(bytes(name).length > 0 && bytes(agency).length > 0, "Invalid info");
    ...
    // Generate simple token URI
    function generateTokenURI(uint256 noticeId) private view returns (string memory) {
        return string(abi.encodePacked("data:application/json,{\"name\":\"Notice #", noticeId.toString(), "\"}"));
    }
