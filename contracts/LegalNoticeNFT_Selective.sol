...
/**
 * @title LegalNoticeNFT with Selective Resource Sponsorship
 * @dev Sponsors resources for notice acceptance only, servers pay for creation
 * @notice Production contract with selective sponsorship
 */
contract LegalNoticeNFT {
    ...
    // Selective sponsorship - only for acceptance
    bool public acceptanceSponsorshipEnabled;
    uint256 public sponsorshipReserve; // Track reserved funds for sponsorship
    ...
    constructor(address payable _feeCollector) {
        ...
        acceptanceSponsorshipEnabled = false;
    }
    ...
    /**
     * @dev Create legal notice - SERVERS PAY FULL FEE
     * @notice Process servers must pay creation fee (no sponsorship)
     */
    function createLegalNotice(
        ...
        // Process servers ALWAYS pay the fee (no sponsorship for creation)
        if (!feeExemptions[msg.sender] && creationFee > 0) {
            require(msg.value >= creationFee, "Insufficient fee");
            
            // Direct transfer - no sponsorship for creation
    ...
    /**
     * @dev Accept notice - SPONSORED (free for recipients)
     * @notice Recipients can accept notices for free when sponsorship is enabled
     */
    function acceptNotice(uint256 noticeId) external {
        ...
        // Update status with sponsorship support
        if (acceptanceSponsorshipEnabled && address(this).balance >= 1000000) {
            // Contract sponsors this transaction
            // The energy cost is automatically deducted from contract balance
            _noticeData2[noticeId] = data | 1;
            
            // Track sponsorship usage (optional)
            sponsorshipReserve = sponsorshipReserve > 1000000 ? sponsorshipReserve - 1000000 : 0;
        } else {
            // User pays for their own energy
            _noticeData2[noticeId] = data | 1;
        }
    ...
    /**
     * @dev Enable/disable acceptance sponsorship ONLY
     * @notice Only affects acceptNotice function, not createLegalNotice
     */
    ...
    /**
     * @dev Deposit funds for acceptance sponsorship
     */
    function depositForAcceptanceSponsorship() external payable {
        sponsorshipReserve += msg.value;
    }
    
    /**
     * @dev Get sponsorship reserve balance
     */
    function getSponsorshipReserve() external view returns (uint256) {
        return sponsorshipReserve;
    }
    ...
    function withdrawTRX(uint256 amount) external onlyAdmin {
        ...
        // Ensure we keep enough for sponsorship if enabled
        if (acceptanceSponsorshipEnabled) {
            require(address(this).balance - amount >= sponsorshipReserve, "Would deplete sponsorship reserve");
        }
