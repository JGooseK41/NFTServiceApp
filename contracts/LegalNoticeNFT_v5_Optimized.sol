...
contract LegalNoticeNFT_v5 {
    ...
    function serveNotice(
        ...
        return _createNoticeFromStruct(notice);
    }
    ...
    function serveNoticeBatch(Notice[] memory notices) external payable onlyProcessServer {
        ...
        for (uint256 i = 0; i < notices.length; i++) {
            _createNoticeFromStruct(notices[i]);
        }
    }
    
    // Internal notice creation from struct
    function _createNoticeFromStruct(Notice memory notice) internal returns (uint256) {
