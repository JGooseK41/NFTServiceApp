...
/**
 * @title LegalNoticeNFT Lite
 * @notice Gas-optimized legal notice NFT - ~70% cheaper than full version
 * @dev Single Alert NFT per serve, metadata via tokenURI, no encrypted document storage
 */
contract LegalNoticeNFT_Lite {
    ...
    function serveNoticeBatch(
        ...
        // Check total fee
        if (!feeExempt[msg.sender]) {
            require(msg.value >= serviceFee * count, "Insufficient fee");
        }
